...
bool MemoryManager::initialize() {
    ...
    if (!initializeMemoryLimit()) {
        return false;
    }

    initializeUVMConnection();
    g_gvm_initialized = true;
    return true;
}

bool MemoryManager::initializeMemoryLimit() {
    ...
    g_memory_limit = memory_limit_gb * 1024LL * 1024LL * 1024LL; // Convert GB to bytes
    return true;
}

bool MemoryManager::initializeUVMConnection() {
    ...
    std::cout << "[GVM] Successfully set GPU memory limit to "
              << g_memory_limit / (1024LL * 1024LL * 1024LL)
...
bool MemoryManager::canAllocate(size_t size) {
    ...
    if (g_cuda_mem_allocated + size > effective_limit) {
        if (g_allow_next_overcommit.load() && g_failed_allocation_size.load() == size) {
            // This is a retry after garbage collection with the same size - allow overcommit
            ...
            // First attempt or different size - return false to trigger PyTorch garbage collection
...
void MemoryManager::recordAllocation(void* ptr, size_t size) {
    ...
    int64_t effective_limit = g_memory_limit > 0 ? g_memory_limit : g_cuda_mem_total;
...
void MemoryManager::getMemoryInfo(size_t* free, size_t* total, size_t actual_free, size_t actual_total) {
